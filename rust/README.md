...
A self-contained, single-binary Rust port of the `skills` CLI whose TypeScript
source lives in [`../src`](../src). The TypeScript code is unchanged and remains
the reference implementation; this port tracks its behavior command-for-command
...
Each module ports the TypeScript file of the same name:
...
[`parity/parity.ps1`](parity/parity.ps1) (PowerShell 7, cross-platform) runs the TypeScript CLI
(`node ../src/cli.ts`) and the Rust binary in fresh, identical sandboxes
(isolated `HOME`/`USERPROFILE`/`TEMP`, telemetry off, agent-detection variables
removed) and compares exit codes, stdout, stderr, and the resulting file trees —
including symlinks/junctions and lock-file contents. Two local HTTP servers
serve well-known indexes and archive downloads so those flows are covered
offline.

```bash
pnpm install && pnpm build        # once, at the repo root (update cases need dist/)
cargo build --release --manifest-path rust/Cargo.toml
pwsh rust/parity/parity.ps1                        # offline cases
pwsh rust/parity/parity.ps1 -Network               # + GitHub / skills.sh cases
pwsh rust/parity/parity.ps1 -Filter add -ShowOutput  # filter, show outputs
```

Current status: 89/89 cases pass (78 offline, 11 network) on Windows. Network cases
