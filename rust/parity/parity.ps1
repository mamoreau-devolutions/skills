...
<#
.SYNOPSIS
    Side-by-side parity checks: the TypeScript CLI (node ../src/cli.ts) vs a native port
    (the Rust port by default; pass -Bin for another build such as the C# port).
...
.DESCRIPTION
    ...
    Requires node >= 22.18 with the TypeScript dependencies installed and built
    (pnpm install && pnpm build at the repo root; update cases need dist/), and a
    port binary: the Rust release build by default, or any binary passed with -Bin
    (dotnet/parity/parity.ps1 runs these cases against the C# port).

.EXAMPLE
    pwsh rust/parity/parity.ps1                      # offline cases
.EXAMPLE
    pwsh rust/parity/parity.ps1 -Network             # also GitHub / skills.sh cases
.EXAMPLE
    pwsh rust/parity/parity.ps1 -Filter add -ShowOutput
#>
[CmdletBinding()]
param(
...
$Repo = Split-Path -Parent $RustDir
if (-not $Bin) {
...
$tsCommand = @($Node, (Join $Repo 'src/cli.ts'))
$rsCommand = @($Bin)
$passed = 0
