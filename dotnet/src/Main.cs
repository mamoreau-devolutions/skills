...
//
// A self-contained C# port of the TypeScript CLI in `../../src` (entry point:
// `../../src/cli.ts`). File names mirror the TS modules they port.
...
internal static class Program
{
    ...
    private static void ShowBanner()
    {
        ...
        Sys.OutLine();
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills add {Dim}<package>{Reset}        {Dim}Add a new skill{Reset}");
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills use {Dim}<package>@<skill>{Reset} {Dim}Use a skill without installing{Reset}");
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills remove{Reset}               {Dim}Remove installed skills{Reset}");
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills list{Reset}                 {Dim}List installed skills{Reset}");
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills find {Dim}[query]{Reset}         {Dim}Search for skills{Reset}");
        Sys.OutLine();
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills update{Reset}               {Dim}Update installed skills{Reset}");
        Sys.OutLine();
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills experimental_install{Reset} {Dim}Restore from skills-lock.json{Reset}");
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills init {Dim}[name]{Reset}          {Dim}Create a new skill{Reset}");
        Sys.OutLine($"  {Dim}${Reset} {Text}npx skills experimental_sync{Reset}    {Dim}Sync skills from node_modules{Reset}");
        Sys.OutLine();
        Sys.OutLine($"{Dim}try:{Reset} npx skills add vercel-labs/agent-skills");
        Sys.OutLine();
    ...
    private static void RunInit(IReadOnlyList<string> args)
    {
        ...
        Sys.OutLine($"  {Dim}GitHub:{Reset}  Push to a repo, then {Text}npx skills add <owner>/<repo>{Reset}");
        Sys.OutLine($"  {Dim}URL:{Reset}     Host the file, then {Text}npx skills add https://example.com/{display}{Reset}");
        Sys.OutLine();
